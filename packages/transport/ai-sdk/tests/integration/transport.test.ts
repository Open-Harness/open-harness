...
import { OpenHarnessChatTransport } from "../../src/transport.js";
import { MockRuntime } from "../helpers/mock-runtime.js";

describe("OpenHarnessChatTransport", () => {
	let mockRuntime: MockRuntime;
	let transport: OpenHarnessChatTransport;

	beforeEach(() => {
		mockRuntime = new MockRuntime();
		transport = new OpenHarnessChatTransport(mockRuntime as unknown as import("@open-harness/sdk").Runtime);
	});
	...
	describe("sendMessages", () => {
		test("extracts last user message and dispatches to runtime", async () => {
			const messages: UIMessage[] = [
				...
				{ id: "3", role: "user" as const, parts: [{ type: "text", text: "World" }] },
			];
		...
		test("throws error when no user message found", async () => {
			const messages: UIMessage[] = [{ id: "1", role: "assistant" as const, parts: [] }];
		...
		test("throws error when user message has no text part", async () => {
			const messages: UIMessage[] = [{ id: "1", role: "user" as const, parts: [] }];
		...
		test("transforms text events to chunks", async () => {
			const messages: UIMessage[] = [{ id: "1", role: "user" as const, parts: [{ type: "text", text: "Hello" }] }];
		...
		test("emits error chunk on dispatch failure", async () => {
			...
			const transportWithError = new OpenHarnessChatTransport(
	...
	describe("reconnectToStream", () => {
		test("returns null", async () => {
			...
			const result = await (transport.reconnectToStream as () => Promise<null>)();
	...
	describe("with real captured events", () => {
		const fixturePath = resolve(process.cwd(), "tests/fixtures/runtime-events.json");
		const events = JSON.parse(readFileSync(fixturePath, "utf-8")) as RuntimeEvent[];
		...
		test("processes full real event sequence", async () => {
			...
			const textChunks = chunks.filter(
				(c) =>
					...
					c !== null &&
					"type" in c &&
					(c.type === "text-start" || c.type === "text-delta" || c.type === "text-end"),
			);
			...
			const toolChunks = chunks.filter(
				(c) =>
					...
					c !== null &&
					"type" in c &&
					(c.type === "tool-input-available" || c.type === "tool-output-available"),
			);
