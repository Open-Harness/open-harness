...
import type { ExpressionError } from "../expressions/errors.js";
...
/**
 * Convert an ExpressionError to an ExecutionError for consistent error handling.
 *
 * @internal
 */
function convertExpressionErrorToExecutionError(
	exprError: ExpressionError,
	message: string,
	nodeId?: string,
	runId?: string,
): ExecutionError {
	return new ExecutionError("INPUT_VALIDATION_ERROR", message, exprError, nodeId, runId);
}
...
class InMemoryRuntime implements Runtime {
	...
	async run(input: Record<string, unknown> = {}): Promise<RunSnapshot> {
		...
		while (this.snapshot.status === "running") {
			...
			const shouldRunResult = await evaluateWhenResult(node.when, bindingContext);
			if (shouldRunResult.isErr()) {
				// Convert ExpressionError to ExecutionError
				const exprError = shouldRunResult.error;
				const execError = convertExpressionErrorToExecutionError(
					exprError,
					`Failed to evaluate 'when' condition: ${exprError.message}`,
					nodeId,
				);
				this.snapshot.nodeStatus[nodeId] = "failed";
				this.snapshot.outputs[nodeId] = { error: execError };
				this.emit({
					type: "node:error",
					nodeId,
			...
			const shouldRun = shouldRunResult.value;
			if (!shouldRun) {
			...
			const resolvedInputResult = await resolveBindingsResult(node.input, bindingContext);
			if (resolvedInputResult.isErr()) {
				// Convert ExpressionError to ExecutionError
				const exprError = resolvedInputResult.error;
				const execError = convertExpressionErrorToExecutionError(
					exprError,
					`Failed to resolve bindings: ${exprError.message}`,
					nodeId,
					runContext.runId,
				);
				this.snapshot.nodeStatus[nodeId] = "failed";
	...
	private async evaluateOutgoingEdges(
		...
		for (const edge of compiled.edges) {
			...
			const shouldFireResult = await evaluateWhenResult(edge.when, bindingContext);
			if (shouldFireResult.isErr()) {
				// Log error but don't crash - mark edge as skipped
				const key = edgeKey(edge);
				this.snapshot.edgeStatus[key] = "skipped";
				// Note: No edge:error event type exists, so we skip the edge silently
				continue;
			}
			const shouldFire = shouldFireResult.value;
			const key = edgeKey(edge);
	...
	private async runForEachNode({
		...
		const bindingContext = this.createBindingContext(input);
		const resolvedResult = await resolveBindingsResult({ value: forEach.in }, bindingContext);
		if (resolvedResult.isErr()) {
			// Convert ExpressionError to ExecutionError
			const exprError = resolvedResult.error;
			throw convertExpressionErrorToExecutionError(
				exprError,
				`Failed to resolve forEach binding: ${exprError.message}`,
				node.id,
			);
		}
		...
		for (let i = 0; i < list.length; i++) {
			...
			const shouldRunResult = await evaluateWhenResult(node.when, iterationContext);
			if (shouldRunResult.isErr()) {
				// Handle error gracefully - skip iteration with error
				const exprError = shouldRunResult.error;
				iterations.push({
					item,
					error: convertExpressionErrorToExecutionError(
						exprError,
						`Failed to evaluate 'when' condition in iteration ${i}: ${exprError.message}`,
						node.id,
					).message,
				});
				continue;
			...
			const resolvedInputResult = await resolveBindingsResult(node.input, iterationContext);
			if (resolvedInputResult.isErr()) {
				// Handle error gracefully - skip iteration with error
				const exprError = resolvedInputResult.error;
				iterations.push({
					...
					error: convertExpressionErrorToExecutionError(
						exprError,
						`Failed to resolve bindings in iteration ${i}: ${exprError.message}`,
						node.id,
						runContext.runId,
					).message,
				});
