...
## Milestone 3: FlowRuntime Lifecycle + Phase/Task Helpers

**Goal**: Implement FlowRuntime lifecycle events, phase/task helpers, and attachments.

**Inputs**:
- `docs/spec/flow-runtime.md` - FlowRuntime protocol
- `tests/specs/flow-runtime.test-spec.md` - FlowRuntime test requirements

**Outputs**:
- `src/flow/runtime.ts` - FlowRuntime implementation
- Replay tests: `tests/replay/flow.runtime.test.ts`
- Fixtures: `tests/fixtures/golden/flow/runtime-*.jsonl`
- Authoritative script: `scripts/live/flow-runtime-live.ts`
...
**Authoritative Script**: `bun scripts/live/flow-runtime-live.ts`

**Done Criteria**: Can create FlowRuntime, run a flow, see lifecycle events, use phase/task helpers. Attachments work. All tests pass.
...
**Inputs**:
- `docs/spec/flow-runtime.md` - Inbox routing requirements
- `docs/spec/agent.md` - AgentInbox interface
- `tests/specs/flow-runtime.test-spec.md` - Inbox routing requirements
- `tests/specs/agent.test-spec.md` - Inbox interface, RunId uniqueness

**Outputs**:
- `src/flow/runtime.ts` - Inbox routing implementation
- `src/engine/inbox.ts` - AgentInbox implementation
- Replay tests: `tests/replay/flow.runtime.test.ts`, `tests/replay/agent.inbox.test.ts`
- Fixtures: `tests/fixtures/golden/flow/runtime-*.jsonl`, `tests/fixtures/golden/agent/*.jsonl`
- Authoritative script: `scripts/live/flow-runtime-live.ts` (updated)
...
End-to-end validation scripts that prove the full stack works with the real Claude SDK.

### `scripts/real-claude-test.ts`

**Purpose**: Raw SDK isolation test - verifies the Claude Code SDK works independently.

```bash
timeout 60 bun scripts/real-claude-test.ts
```

**What it tests**:
- Direct `query()` call to Claude SDK
- Response streaming and result extraction
- No FlowRuntime involved - pure SDK validation

### `scripts/real-flow-test.ts`

**Purpose**: Full stack validation - proves FlowRuntime → executor → claude.agent node → SDK pipeline works.

```bash
timeout 90 bun scripts/real-flow-test.ts
```

**What it tests**:
- YAML flow parsing and execution
- Node registry with real `claudePack`
- Claude provider calling real SDK
- Response propagation through the stack

**Expected output**: `✓ SUCCESS - Claude responded: Hello from FlowRuntime`

### When to use these scripts

| Script | Use Case |
|--------|----------|
| `real-claude-test.ts` | Debugging SDK issues, verifying SDK works in isolation |
| `real-flow-test.ts` | Validating full integration after provider changes |

**Note**: These scripts make real API calls using Claude Code subscription auth. They are not part of the regular test suite (which uses mocks).
