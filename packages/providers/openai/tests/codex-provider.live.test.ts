...
import { PROVIDER_SIGNALS } from "@signals/core";
...
describe("CodexProvider Live Integration", () => {
	describe("basic query", () => {
		test.skipIf(!isLiveTest())(
			"records simple text response",
			async () => {
				const provider = new CodexProvider({ model: FAST_MODEL });
	...
	describe("session management", () => {
		test.skipIf(!isLiveTest())(
			...
			async () => {
				...
				// sessionId should also be set (for Provider interface compatibility)
	...
	describe("source tracking", () => {
		test.skipIf(!isLiveTest())(
			"all signals have source.provider = codex",
			async () => {
				const provider = new CodexProvider({ model: FAST_MODEL });
...
describe("CodexProvider Fixture Replay", () => {
