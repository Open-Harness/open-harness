...
describe("transformTextEvent", () => {
	...
	test("subsequent delta emits only delta", () => {
		...
		const chunks = transformTextEvent(
			{ type: "agent:text:delta", nodeId: "n1", runId: "r1", content: " world" },
			tracker,
			"msg-1",
		);

		expect(chunks).toEqual([{ type: "text-delta", id: "msg-1", delta: " world" }]);
	});
...
describe("transformToolEvent", () => {
	test("emits input-available and output-available chunks", () => {
		...
		expect((chunks[0] as { toolCallId: string }).toolCallId).toBe((chunks[1] as { toolCallId: string }).toolCallId);
	});
...
describe("transformStepEvent", () => {
	test("emits step-start when sendStepMarkers is true", () => {
		const chunks = transformStepEvent({ type: "node:start" }, { sendStepMarkers: true });
	...
	test("returns empty array when sendStepMarkers is false", () => {
		const chunks = transformStepEvent({ type: "node:start" }, { sendStepMarkers: false });
...
describe("transformErrorEvent", () => {
	...
	test("emits error chunk with error from node:error", () => {
		...
		expect(chunks).toEqual([{ type: "error", errorText: "Node execution failed" }]);
	});
...
describe("transformEvent with real captured events", () => {
	...
	test("transforms real agent:thinking:delta events", () => {
		...
		const thinkingDeltaEvents = events.filter((e) => e.type === "agent:thinking:delta");
