 ...
 */
export async function timeTravel(destination: Date, options: TimeTravelOptions = {}): Promise<TimeTravelResponse> {
...
function validateDateInput(date: Date): void {
	...
	if (year > FAR_FUTURE_LIMIT) {
		throw new TimeTravelError(
			`Year ${year} exceeds our maximum simulation range. ` + `Please choose a date before ${FAR_FUTURE_LIMIT} CE.`,
	...
	if (month === 1 && day === 29 && !isLeapYear(year)) {
		throw new TimeTravelError(
			`February 29, ${year} is invalid because ${year} is not a leap year. Please choose a valid date.`,
			"INVALID_LEAP_DATE",
		);
	}
	...
	if (day > getDaysInMonth(month, year)) {
		throw new TimeTravelError(
			`Day ${day} is invalid for ${getMonthName(month)} ${year}. Please choose a valid date.`,
