/**
 * ClaudeHarness - Harness implementation for Claude SDK
 *
...
/**
 * Claude-specific harness input extensions
 */
export interface ClaudeHarnessInput extends HarnessInput {
...
/**
 * Claude-specific harness output extensions
 */
export interface ClaudeHarnessOutput extends HarnessOutput {
...
/**
 * Configuration for ClaudeHarness
 */
...
// ============================================================================
// ClaudeHarness Implementation
// ============================================================================

/**
 * Harness for Claude SDK
 *
 * Implements the Harness interface for @anthropic-ai/claude-agent-sdk.
 ...
 * ```
 */
export class ClaudeHarness implements Harness<ClaudeHarnessInput, ClaudeHarnessOutput> {
	...
	constructor(config?: ClaudeHarnessConfig) {
	 ...
	 */
	async *run(input: ClaudeHarnessInput, ctx: RunContext): AsyncGenerator<Signal, ClaudeHarnessOutput> {
		const startTime = Date.now();
		const source = { harness: this.type };
		...
		let stopReason: HarnessOutput["stopReason"] = "end";
		...
		if (accumulatedThinking) {
			yield createSignal(
				HARNESS_SIGNALS.THINKING_COMPLETE,
				{
		...
		// Build final output
		const output: ClaudeHarnessOutput = {
			content: accumulatedText,
			toolCalls: toolCalls.length > 0 ? toolCalls : undefined,
			sessionId,
			usage,
	...
	private *processMessage(
		...
		onStopReason: (r: HarnessOutput["stopReason"]) => void,
	): Generator<Signal> {
		...
		if (sdkMessage.type === "stream_event") {
			...
			if (streamEvent?.type === "content_block_delta") {
				...
				if (delta?.type === "thinking_delta" && delta.thinking) {
					...
					yield createSignal(
						HARNESS_SIGNALS.THINKING_DELTA,
						{
	...
	/**
	 * Convert HarnessInput messages to SDK format
	 */
	private async *toSDKMessages(input: ClaudeHarnessInput): AsyncGenerator<SDKUserMessage> {
