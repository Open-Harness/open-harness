...
function simulateAnalysisWorkflow(): Signal[] {
	return [
		...
		createSignal("harness:start", { harness: "claude" }),
		createSignal("harness:end", {
...
describe("Testing Reactive Workflows", () => {
	...
	describe("toHaveSignalCount - Counting Signals", () => {
		it("counts exact signal occurrences", () => {
			...
			// Two harness calls
			expect(signals).toHaveSignalCount("harness:start", 2);
			expect(signals).toHaveSignalCount("harness:end", 2);
		});

		it("counts signals matching glob patterns", () => {
			// All harness lifecycle signals (start + end = 4)
	...
	describe("toHaveSignalsInOrder - Sequence Validation", () => {
		...
		it("validates harness lifecycle ordering", () => {
			// Harness signals must be properly paired
			expect(signals).toHaveSignalsInOrder(["harness:start", "harness:end", "harness:start", "harness:end"]);
		});
		...
		it("works with glob patterns in sequence", () => {
			expect(signals).toHaveSignalsInOrder(["workflow:*", "agent:*", "harness:*", "*:complete", "workflow:*"]);
		});
	});
});
...
describe("Testing Error Scenarios", () => {
	function simulateFailedWorkflow(): Signal[] {
		return [
			createSignal("workflow:start", { agents: ["analyst"] }),
			createSignal("agent:activated", { agent: "analyst" }),
			createSignal("harness:start", { harness: "claude" }),
			createSignal("harness:error", {
	...
	it("detects error signals", () => {
		...
		expect(signals).toContainSignal("harness:error");
		expect(signals).toContainSignal({
			name: "harness:error",
	...
	it("validates error flow sequence", () => {
		...
		expect(signals).toHaveSignalsInOrder(["workflow:start", "harness:start", "harness:error", "workflow:end"]);

		// Should NOT contain successful completion
		expect(signals).not.toContainSignal("harness:end");
	...
	it("counts partial completions", () => {
		...
		// Only one harness call started
		expect(signals).toHaveSignalCount("harness:start", 1);

		// No successful harness completions
		expect(signals).toHaveSignalCount("harness:end", 0);

		// One error occurred
		expect(signals).toHaveSignalCount("harness:error", 1);
	});
});
...
describe("Testing Parallel Execution", () => {
	function simulateParallelAgents(): Signal[] {
		return [
			...
			// Both agents activate on workflow:start (parallel)
			createSignal("agent:activated", {
				agent: "analyst",
				trigger: "workflow:start",
			}),
			...
			// Interleaved harness calls (parallel execution)
			createSignal("harness:start", { agent: "analyst" }),
			createSignal("harness:start", { agent: "risk-assessor" }),

			createSignal("harness:end", { agent: "risk-assessor" }),
			createSignal("harness:end", { agent: "analyst" }),

			// Both complete
			createSignal("analysis:complete", { agent: "analyst" }),
			createSignal("risk:assessed", { agent: "risk-assessor" }),

			createSignal("workflow:end", { activations: 2 }),
		];
	}
	...
	it("detects parallel agent activations", () => {
		...
		expect(signals).toHaveSignalCount({ name: "agent:activated", payload: { trigger: "workflow:start" } }, 2);
	});
	...
	it("confirms parallel harness calls", () => {
		const signals = simulateParallelAgents();

		// Two harness calls made
		expect(signals).toHaveSignalCount("harness:start", 2);
		expect(signals).toHaveSignalCount("harness:end", 2);
	});
});
