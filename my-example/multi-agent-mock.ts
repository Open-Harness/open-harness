import { executeFlow, createHub, NodeRegistry } from "../packages/kernel/src/index.js";
import { loadFlowYamlFile } from "../packages/kernel/src/flow/loader.js";

// Import mock Claude pack for reliable demo without API calls
import { mockClaudePack } from "../packages/kernel-tutorial/src/packs/tutorial-pack.js";
...
async function main() {
  ...
  hub.subscribe((event) => {
    console.log(`[Event] ${String(event.type)}`);
  });

  const registry = new NodeRegistry();
  mockClaudePack.register(registry);
