...
import type { CCXTInterface, OHLCV, OrderResult, Position, Balance } from './ccxt-interface'
...
export class MockCCXT implements CCXTInterface {
  ...
  private updatePosition(symbol: string, side: 'buy' | 'sell', amount: number, price: number): void {
    ...
    if (existing) {
      ...
      const newContracts = existing.side === positionSide
        ? existing.contracts + amount
        : existing.contracts - amount
