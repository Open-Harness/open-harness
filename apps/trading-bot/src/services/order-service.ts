...
export class OrderService {
	...
	async placeOrder(request: OrderRequest): Promise<OrderResult> {
		...
		const result = await this.ccxt.createOrder(request.symbol, type, side, request.size, request.price);
	...
	async closePosition(symbol: string): Promise<OrderResult | null> {
		...
		const result = await this.ccxt.createOrder(symbol, "market", side, position.contracts);
	...
	private closeTrade(symbol: string, result: OrderResult): void {
		...
		if (trades.length > 0) {
			...
			this.db.run(`UPDATE trades SET exit_price = ?, profit = ?, status = 'closed', closed_at = ? WHERE id = ?`, [
				result.price,
				profit,
				this.timeSource.now(),
				trade.id,
