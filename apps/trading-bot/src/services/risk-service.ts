...
import type { CCXTInterface, Position } from '../ccxt/ccxt-interface'
...
export class RiskService {
  ...
  async validateDCA(symbol: string, size: number): Promise<ValidationResult> {
  ...
  private async getPositionAge(symbol: string): Promise<{ createdAt: number; unrealizedPnl: number } | null> {
    const rows = this.db.query<{ created_at: number }>(
      'SELECT created_at FROM positions WHERE symbol = ? LIMIT 1',
      [symbol]
    )
