...
import type { AgentState, MonologueEntry, Snapshot } from "./agent-state";
...
export class SnapshotStorage {
	...
	async restore(id: string): Promise<Snapshot | null> {
		const rows = this.db.query<{ json_path: string }>("SELECT json_path FROM snapshots WHERE id = ?", [id]);
	...
	async list(
		options: { limit?: number; stage?: string; after?: number; before?: number } = {},
