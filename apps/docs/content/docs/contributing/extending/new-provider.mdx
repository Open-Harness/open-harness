...
The Claude provider is the reference implementation:
...
export function createClaudeAgent(
  ...
  const client = new Client();  // Uses Claude Code subscription auth
...
```typescript
// src/index.ts
export { createClaudeAgent } from "./providers/claude";
...
New providers should match the Claude API surface:
...
- [Claude Agent](/docs/reference/api/anthropic-agent) - Reference implementation
