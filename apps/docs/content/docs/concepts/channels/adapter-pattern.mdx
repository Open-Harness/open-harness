...
One flow, many destinations:

```typescript
// Attach channels to the hub
hub.attach(createChannelAttachment(terminalChannel));
hub.attach(createChannelAttachment(logChannel));
hub.attach(createChannelAttachment(metricsChannel));
```
