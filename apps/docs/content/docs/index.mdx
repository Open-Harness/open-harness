...
const analyzer = agent({
  ...
  activateOn: ["harness:start"],  // React to harness start
...
// Signals: harness:start → agent:activated → provider:end → analysis:complete → harness:end
```
...
| Concept | Description |
|---------|-------------|
| **Signal** | An event that flows through the system (`harness:start`, `analysis:complete`) |
| **Agent** | A reactive unit that subscribes to signals and produces output |
| **Harness** | A container that coordinates agents and manages state |
| **Provider** | An AI model adapter (Claude, OpenAI) that executes agent prompts |
...
<Cards>
  ...
  <Card title="Architecture" href="/docs/concepts/architecture">
    Understand signals, agents, and harnesses
  </Card>
...
// Activate on provider events (advanced)
