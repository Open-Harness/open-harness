...
Defines a channel adapter that can observe events and inject commands into a flow.
...
| `onStart` | `function` | No | Called when flow starts |
| `onComplete` | `function` | No | Called when flow completes |
...
Channels are attached to the Hub using `createChannelAttachment`:

```typescript
import { createHub, createChannelAttachment } from "@open-harness/kernel";

const hub = createHub();

// Attach channels
hub.attach(createChannelAttachment(consoleChannel));
hub.attach(createChannelAttachment(loggerChannel));

// Execute flow with the hub
await executeFlow(flow, { hub, registry, input });
```
...
Channels return a cleanup function that's called when the flow completes.
...
- [Hub](/docs/reference/api/hub) - Event bus API
- [Attachment](/docs/reference/types/attachment) - Attachment type
- [Channel Spec](/docs/reference/kernel-spec/spec/channel) - Specification
