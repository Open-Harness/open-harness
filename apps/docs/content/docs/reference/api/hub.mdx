...
Abort or pause the current session.
...
hub.abort({ reason: "User requested pause", resumable: true });
```
...
interface PauseOptions {
  reason?: string;      // Optional reason for abort/pause
  resumable?: boolean;  // If true, session can be resumed later
}
```

### resume

Resume a paused session with a user message.

```typescript
await hub.resume("session-123", "Continue with the analysis");
```

Throws `SessionNotFoundError` if session doesn't exist or `SessionAlreadyRunningError` if already running.

### getPausedSession

Get the state of a paused session.

```typescript
const state = hub.getPausedSession("session-123");
if (state) {
  console.log(`Paused at node: ${state.currentNodeId}`);
  console.log(`Paused at: ${state.pausedAt}`);
}
```
