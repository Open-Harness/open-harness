  ...
  useEffect(() => {
    const unsubscribe = subscribe((event) => {
      ...
      if (event.type === "node:start") {
        ...
        setNodeExecutionState(event.event.nodeId, "success");
...
import { HubImpl, NodeRegistry, executeFlow, parseFlowYaml } from "@open-harness/kernel";
...
const server = createFlowServer({
  ...
  onExecute: async (flowDef, emit) => {
    ...
    // Execute the flow
    await executeFlow(flowDef, registry, ctx);
  },
});
...
- [Components Reference](/docs/reference/flow-ui/components) — All component props and usage
- [Hooks Reference](/docs/reference/flow-ui/hooks) — State management and WebSocket hooks
