...
// Create registry with LLM node using createClaudeAgent
...
registry.register({
  type: "llm",
  async run(ctx, input) {
    const result = await claudeAgent.execute(input, { hub: ctx.hub, runId: ctx.runId });
...
// Execute the flow
const result = await executeFlow(flow, registry, ctx, { name: "World" });
