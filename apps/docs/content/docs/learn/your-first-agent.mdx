...
```mermaid
flowchart LR
    Start([harness:start]) --> C[Classifier]
...
1. **harness:start** triggers the classifier
...
sequenceDiagram
    ...
    H->>C: activate
    C->>H: update classification
    C->>H: emit classify:complete
    H->>U: check when guard → true
    H->>N: check when guard → false (skip)
    U->>H: update response
    U->>H: emit response:complete
    H->>H: endWhen → true
```
