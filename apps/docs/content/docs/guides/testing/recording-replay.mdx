...
- **Slow**: Each test calls the AI provider
...
- **Free**: No provider costs during replay
...
### Provider Not Called During Replay

The provider is not called during replay—signals are returned from the store. You can pass any provider:

```typescript
// Provider is ignored during replay
const replay = await runReactive({
