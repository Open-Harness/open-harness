...
2. **Dispatch research agents in parallel**:
   ...
   ```
   Task: oharnes.plan:researcher
   Prompt: |
     FEATURE_SPEC: {FEATURE_SPEC}
     SPECS_DIR: {SPECS_DIR}
     UNKNOWN: {first unknown from Technical Context}
     CONTEXT: {relevant feature context for this unknown}
     Research this unknown and return a decision with rationale.

   Task: oharnes.plan:researcher
   Prompt: |
     FEATURE_SPEC: {FEATURE_SPEC}
     SPECS_DIR: {SPECS_DIR}
     UNKNOWN: {second unknown from Technical Context}
     CONTEXT: {relevant feature context for this unknown}
     Research this unknown and return a decision with rationale.

   [... one Task per unknown, all in parallel ...]
   ```
