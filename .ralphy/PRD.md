...
- [x] Test live SDK: send simple prompt, receive streaming response, verify text:delta and text:complete events are emitted - test MUST PASS
- [x] Test live SDK: send prompt requiring tool use, verify tool:called and tool:result events match SDK behavior - test MUST PASS
- [x] Test live SDK: send prompt with outputSchema, verify structured output is returned and parsed correctly - test MUST PASS
- [x] Add npm script "test:live" to package.json that runs integration tests with proper vitest config (no skip)
