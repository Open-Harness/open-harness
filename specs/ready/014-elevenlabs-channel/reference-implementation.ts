...
import type { HarnessControl, IUnifiedEventBus } from "@openharness/sdk";
...
class ElevenLabsConnection {
	...
	async connect(): Promise<void> {
		...
		throw new Error(`Failed to connect to ElevenLabs after ${maxAttempts} attempts: ${lastError?.message}`);
	}
		...
		onComplete: ({ state, output }) => {
			...
			if (config.verbosity === "debug") {
				...
				for (const entry of state.transcript) {
					output.line(`  [${entry.timestamp.toISOString()}] ${entry.speaker}: ${entry.text}`);
				}
			}
		},
	});
