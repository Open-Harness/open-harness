...
- [ ] T001 Create event context types in packages/sdk/src/harness/event-context.ts (copy from contracts)
- [ ] T002 [P] Add IUnifiedEventBusToken to packages/sdk/src/core/tokens.ts
- [ ] T003 [P] Create barrel export file packages/sdk/src/core/unified-events/index.ts
...
- [ ] T004 Implement matchesFilter() utility in packages/sdk/src/core/unified-events/filter.ts
- [ ] T005 Implement UnifiedEventBus class skeleton in packages/sdk/src/core/unified-event-bus.ts (constructor with AsyncLocalStorage, sessionId generation)
...
- [ ] T008 Implement UnifiedEventBus.emit() method with auto-context attachment and EnrichedEvent wrapping
...
- [ ] T011 Register UnifiedEventBus in DI container packages/sdk/src/core/container.ts
...
- [ ] T013 [P] [US1] Unit test: context inheritance in scoped() - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T014 [P] [US1] Unit test: nested scopes merge context correctly - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T015 [P] [US1] Unit test: emit() auto-attaches context from AsyncLocalStorage - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T016 [US1] Integration test: agent events include task context (SC-001) - packages/sdk/tests/integration/unified-events.test.ts

### Implementation for User Story 1

- [ ] T017 [US1] Update HarnessInstance.phase() to use UnifiedEventBus.scoped() for phase context - packages/sdk/src/harness/harness-instance.ts
- [ ] T018 [US1] Update HarnessInstance.task() to use UnifiedEventBus.scoped() for task context - packages/sdk/src/harness/harness-instance.ts
- [ ] T019 [US1] Inject UnifiedEventBus into BaseAnthropicAgent - packages/sdk/src/providers/anthropic/runner/base-agent.ts
- [ ] T020 [US1] Update agent event emission to use UnifiedEventBus.emit() - packages/sdk/src/providers/anthropic/runner/base-agent.ts
- [ ] T021 [US1] Create event-mapper.ts to map SDK AgentEvents to unified format - packages/sdk/src/providers/anthropic/runner/event-mapper.ts
...
- [ ] T022 [P] [US2] Unit test: AsyncLocalStorage isolation in Promise.all() - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T023 [US2] Integration test: 3 parallel tasks with correct context (SC-002) - packages/sdk/tests/integration/unified-events.test.ts

### Implementation for User Story 2

- [ ] T024 [US2] Verify scoped() correctly wraps AsyncLocalStorage.run() for parallel safety - packages/sdk/src/core/unified-event-bus.ts
- [ ] T025 [US2] Add stress test for parallel context isolation (10 concurrent tasks) - packages/sdk/tests/integration/unified-events.test.ts
...
- [ ] T026 [P] [US3] Unit test: subscribe('*') receives all event types - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T027 [P] [US3] Unit test: subscribe with type filter works (task:*, agent:*) - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T028 [P] [US3] Unit test: multiple subscribers receive same event - packages/sdk/tests/unit/unified-event-bus.test.ts
...
- [ ] T030 [US3] Ensure subscribe() without filter defaults to '*' - packages/sdk/src/core/unified-event-bus.ts
...
- [ ] T032 [P] [US4] Unit test: defineRenderer() creates valid IUnifiedRenderer - packages/sdk/tests/unit/define-renderer.test.ts
- [ ] T033 [P] [US4] Unit test: renderer state factory called fresh on attach() - packages/sdk/tests/unit/define-renderer.test.ts
- [ ] T034 [P] [US4] Unit test: event handlers receive typed RenderContext - packages/sdk/tests/unit/define-renderer.test.ts
- [ ] T035 [P] [US4] Unit test: onStart/onComplete lifecycle hooks called - packages/sdk/tests/unit/define-renderer.test.ts
- [ ] T036 [US4] Type test: TypeScript inference works with defineRenderer (SC-003) - packages/sdk/tests/unit/define-renderer.types.test.ts

### Implementation for User Story 4

- [ ] T037 [US4] Implement RenderOutput helper class - packages/sdk/src/harness/render-output.ts
- [ ] T038 [US4] Implement defineRenderer() factory function - packages/sdk/src/harness/define-renderer.ts
- [ ] T039 [US4] Implement UnifiedRenderer class that attaches to bus - packages/sdk/src/harness/unified-renderer.ts
- [ ] T040 [US4] Export defineRenderer from packages/sdk/src/index.ts
...
- [ ] T041 [P] [US5] Verify existing harness.on() tests pass unchanged (SC-004) - packages/sdk/tests/harness/
- [ ] T042 [US5] Integration test: legacy .on() and new bus.subscribe() coexist - packages/sdk/tests/integration/unified-events.test.ts

### Implementation for User Story 5

- [ ] T043 [US5] Update HarnessInstance.on() to delegate to UnifiedEventBus.subscribe() - packages/sdk/src/harness/harness-instance.ts
- [ ] T044 [US5] Create legacy event adapter to map unified events back to legacy format - packages/sdk/src/harness/legacy-adapter.ts
- [ ] T045 [US5] Verify legacy HarnessEvent types still exported - packages/sdk/src/index.ts
...
- [ ] T046 [P] [US6] Unit test: EnrichedEvent has id, timestamp, context, event - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T047 [P] [US6] Unit test: event.id is valid UUID - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T048 [US6] Unit test: context override merges with inherited (override wins) - packages/sdk/tests/unit/unified-event-bus.test.ts

### Implementation for User Story 6

- [ ] T049 [US6] Implement UUID generation for event IDs - packages/sdk/src/core/unified-event-bus.ts
...
- [ ] T051 [P] [US7] Unit test: custom context fields preserved in scope - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T052 [P] [US7] Unit test: nested scopes override correctly - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T053 [US7] Unit test: scope that throws still reverts context - packages/sdk/tests/unit/unified-event-bus.test.ts

### Implementation for User Story 7

- [ ] T054 [US7] Verify scoped() handles sync and async functions - packages/sdk/src/core/unified-event-bus.ts
- [ ] T055 [US7] Add error boundary to scoped() to ensure context cleanup - packages/sdk/src/core/unified-event-bus.ts
...
- [ ] T056 [P] Unit test: empty context returns only sessionId - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T057 [P] Unit test: listener throws logs error, other listeners still called - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T058 [P] Unit test: emit after clear() succeeds but no delivery - packages/sdk/tests/unit/unified-event-bus.test.ts
- [ ] T059 [P] Unit test: invalid filter never matches (future-proof) - packages/sdk/tests/unit/unified-event-bus.test.ts
...
- [ ] T062 Add try/catch in emit() to log listener errors without crashing - packages/sdk/src/core/unified-event-bus.ts
...
- [ ] T063 Run coverage check for unified-event-bus.ts (SC-006 >= 90%) - bun test --coverage
- [ ] T064 Create console renderer showing agent activity with task context (SC-007) - packages/sdk/tests/integration/console-renderer.test.ts
- [ ] T065 Verify quickstart.md examples work - run code samples from quickstart.md
- [ ] T066 Run full test suite to verify no regressions - bun run test
...
- [ ] T067 [P] Add JSDoc comments to all public APIs - packages/sdk/src/core/unified-event-bus.ts, packages/sdk/src/harness/define-renderer.ts
- [ ] T068 [P] Run lint and fix any issues - bun run lint
- [ ] T069 [P] Run typecheck and fix any issues - bun run typecheck
- [ ] T070 Review and remove any legacy EventBus forwarding code if Phase 5 migration complete - packages/sdk/src/core/unified-event-bus.ts
